...
namespace library
{
    ...
    HRESULT Shader::compile(ID3DBlob** ppOutBlob)
    {
        ...
        *ppOutBlob = nullptr;
        ...
        HRESULT hr = D3DCompileFromFile(m_pszFileName, defines, D3D_COMPILE_STANDARD_FILE_INCLUDE,
            m_pszEntryPoint, m_pszShaderModel, flags, 0, ppOutBlob, errorBlob.GetAddressOf());
        if (FAILED(hr))
        {
            if (errorBlob)
